import React, { useState, useEffect, createRef } from 'react';
import { Box, styled, withStyles, TextField, Popper } from '@material-ui/core';
import { Checkbox as MUICheckbox } from '../../../UI';
import Autocomplete from '@material-ui/lab/Autocomplete';
import CheckBoxOutlineBlankIcon from '@material-ui/icons/CheckBoxOutlineBlank';
import CheckBoxIcon from '@material-ui/icons/CheckBox';
import { cloneState } from '../../../EstimateProduct/stateClone';
import { getUpdatedSurfaces, countTrueAndFalseOnes } from '../../index';

import {
  useSurfaceSelectionState,
  useSurfaceSelectionDispatch,
  updateSurfaces,
} from '../../../../Context/SurfaceSelectionContext';

import match from 'autosuggest-highlight/match';
import parse from 'autosuggest-highlight/parse';
const icon = <CheckBoxOutlineBlankIcon fontSize="small" />;
const checkedIcon = <CheckBoxIcon fontSize="small" />;

const ListWrapper = styled(Box)({
  paddingTop: '6px',
  paddingLeft: '16px',
  paddingRight: '16px',
  paddingBottom: '6px',
});

const StyledAutoComplete = withStyles({
  root: {
    '& .MuiFormControl-root': {
      width: 200,
    },
  },
  listbox: {
    maxHeight: '30vh',
  },
  option: {
    padding: 0,
  },
  paper: {
    width: '300px',
  },
})(Autocomplete);
const CustomizePopper = function (props) {
  return (
    <Popper
      {...props}
      style={{ width: 300 }}
      placement={'top'}
      modifiers={{
        flip: {
          enabled: false,
        },
      }}
    />
  );
};
export default function SurfaceMultiSelect(props) {
  const {
    estimateSurface,
    openSelectSurface,
    setOpenSelectSurface,
    expanded,
    setExpanded,
    setExpandIcon,
  } = props;
  const realEstimateSurface = useSurfaceSelectionState();
  const surfaceSelectionDispatch = useSurfaceSelectionDispatch();

  const [options, setoptions] = useState([]);
  const [textFieldVal, setTextFieldVal] = useState('');
  const multiSelectTextRef = createRef();

  useEffect(() => {
    setoptions(getOptions(estimateSurface));
  }, [estimateSurface]);

  return (
    <StyledAutoComplete
      multiple
      closeIcon={false}
      autoHighlight
      openOnFocus
      PopperComponent={CustomizePopper}
      disableCloseOnSelect
      onChange={(e, allValues, type, value) => {
        const selectedOption = value.option;
        let selectedValue = false;
        if (!selectedOption['selected']) selectedValue = true;

        const highestPosition = Math.max(
          ...options
            .filter(
              (surface) =>
                selectedOption.parent_id == surface.parent_id &&
                surface.position >= 0
            )
            .map((surface) => surface.position)
        );
        const position = selectedValue
          ? highestPosition >= 0
            ? highestPosition + 1
            : 0
          : undefined;

        if (!expanded) {
          setExpanded(true);
          setExpandIcon('Less');
        }
        const updatedSurfaces = getUpdatedSurfaces(
          'selected',
          selectedValue,
          selectedOption.id,
          cloneState(realEstimateSurface),
          position
        ).obj;

        updateSurfaces(surfaceSelectionDispatch, updatedSurfaces);
        multiSelectTextRef.current.childNodes[1].children[0].select();
      }}
      options={options}
      getOptionLabel={(option) => option.name}
      getOptionSelected={(option, value) => {
        return option.selected;
      }}
      filterOptions={(options, { inputValue, selected }) => {
        if (inputValue != '') {
          let union = [];
          options = options.filter((option) =>
            option.name.toLowerCase().includes(inputValue.toLowerCase())
          );
          options.map((o) => {
            union.push(o);
            let childSurfaces = [];
            if (o.surfaces && o.surfaces.length > 0) {
              childSurfaces = getOptions(o.surfaces);
              childSurfaces = childSurfaces.map((c) => ({
                ...c,
                numOfParent: o.numOfParent + 1,
              }));
            }
            union = union.concat(childSurfaces).filter(function (c) {
              return this.has(c.id) ? false : this.add(c.id);
            }, new Set());
          });
          return union;
        } else return options;
      }}
      blurOnSelect={false}
      value={options.filter((o) => o.selected)}
      size="small"
      renderOption={(option, { selected, inputValue }) => {
        const matches = match(option.name, inputValue);
        const parts = parse(option.name, matches);

        return (
          <ListWrapper
            style={{
              background: pSBC(option.numOfParent / -20, 'rgb(255,255,255)'),
              width: '100%',
              height: '100%',
            }}
          >
            <MUICheckbox
              icon={icon}
              checkedIcon={checkedIcon}
              style={{ marginRight: 8 }}
              checked={option.selected || option.indeterminate ? true : false}
              onChange={(e) => {
                option.indeterminate && (e.target.checked = false);
                const highestPosition = Math.max(
                  ...options
                    .filter(
                      (surface) =>
                        option.parent_id == surface.parent_id &&
                        surface.position >= 0
                    )
                    .map((surface) => surface.position)
                );
                const position = e.target.checked
                  ? highestPosition >= 0
                    ? highestPosition + 1
                    : 0
                  : undefined;
                if (!expanded) {
                  setExpanded(true);
                  setExpandIcon('Less');
                }
                const updatedSurfaces = getUpdatedSurfaces(
                  'selected',
                  e.target.checked,
                  option.id,
                  cloneState(realEstimateSurface),
                  position
                ).obj;

                updateSurfaces(surfaceSelectionDispatch, updatedSurfaces);
              }}
            />
            <span
              style={{
                paddingLeft: option.numOfParent * 20,
              }}
            >
              {parts.map((part, index) => (
                <span
                  key={index}
                  style={{ ...(part.highlight && { color: '#1488FC' }) }}
                >
                  {part.text}
                </span>
              ))}
            </span>
          </ListWrapper>
        );
      }}
      inputValue={textFieldVal}
      onInputChange={(event, newInputValue, reason) => {
        setTextFieldVal(reason === 'input' ? newInputValue : textFieldVal);
      }}
      renderTags={() => {}}
      renderInput={(params) => (
        <TextField
          ref={multiSelectTextRef}
          onBlur={() => openSelectSurface && setOpenSelectSurface(false)}
          autoFocus={true}
          {...params}
          variant="outlined"
          label="Sub-surfaces"
          InputProps={{
            ...params.InputProps,
          }}
        />
      )}
    />
  );
}

const getOptions = (estimateSurface, outputArr = [], numOfParent = 0) => {
  for (let c1 = 0; c1 < estimateSurface.length; c1++) {
    outputArr.push({ ...estimateSurface[c1], numOfParent });
    if (
      estimateSurface[c1].surfaces &&
      estimateSurface[c1].surfaces.length > 0
    ) {
      getOptions(estimateSurface[c1].surfaces, outputArr, numOfParent + 1);
    }
  }
  return outputArr;
};

const pSBC = (p, c0, c1, l) => {
  let r,
    g,
    b,
    P,
    f,
    t,
    h,
    i = parseInt,
    m = Math.round,
    a = typeof c1 == 'string';
  if (
    typeof p != 'number' ||
    p < -1 ||
    p > 1 ||
    typeof c0 != 'string' ||
    (c0[0] != 'r' && c0[0] != '#') ||
    (c1 && !a)
  )
    return null;
  (h = c0.length > 9),
    (h = a ? (c1.length > 9 ? true : c1 == 'c' ? !h : false) : h),
    (f = pSBCr(c0, r, g, b, a, i)),
    (P = p < 0),
    (t =
      c1 && c1 != 'c'
        ? pSBCr(c1, r, g, b, a, i)
        : P
        ? { r: 0, g: 0, b: 0, a: -1 }
        : { r: 255, g: 255, b: 255, a: -1 }),
    (p = P ? p * -1 : p),
    (P = 1 - p);
  if (!f || !t) return null;
  if (l)
    (r = m(P * f.r + p * t.r)),
      (g = m(P * f.g + p * t.g)),
      (b = m(P * f.b + p * t.b));
  else
    (r = m((P * f.r ** 2 + p * t.r ** 2) ** 0.5)),
      (g = m((P * f.g ** 2 + p * t.g ** 2) ** 0.5)),
      (b = m((P * f.b ** 2 + p * t.b ** 2) ** 0.5));
  (a = f.a),
    (t = t.a),
    (f = a >= 0 || t >= 0),
    (a = f ? (a < 0 ? t : t < 0 ? a : a * P + t * p) : 0);
  if (h)
    return (
      'rgb' +
      (f ? 'a(' : '(') +
      r +
      ',' +
      g +
      ',' +
      b +
      (f ? ',' + m(a * 1000) / 1000 : '') +
      ')'
    );
  else
    return (
      '#' +
      (4294967296 + r * 16777216 + g * 65536 + b * 256 + (f ? m(a * 255) : 0))
        .toString(16)
        .slice(1, f ? undefined : -2)
    );
};

const pSBCr = (d, r, g, b, a, i) => {
  let n = d.length,
    x = {};
  if (n > 9) {
    ([r, g, b, a] = d = d.split(',')), (n = d.length);
    if (n < 3 || n > 4) return null;
    (x.r = i(r[3] == 'a' ? r.slice(5) : r.slice(4))),
      (x.g = i(g)),
      (x.b = i(b)),
      (x.a = a ? parseFloat(a) : -1);
  } else {
    if (n == 8 || n == 6 || n < 4) return null;
    if (n < 6)
      d =
        '#' +
        d[1] +
        d[1] +
        d[2] +
        d[2] +
        d[3] +
        d[3] +
        (n > 4 ? d[4] + d[4] : '');
    d = i(d.slice(1), 16);
    if (n == 9 || n == 5)
      (x.r = (d >> 24) & 255),
        (x.g = (d >> 16) & 255),
        (x.b = (d >> 8) & 255),
        (x.a = m((d & 255) / 0.255) / 1000);
    else (x.r = d >> 16), (x.g = (d >> 8) & 255), (x.b = d & 255), (x.a = -1);
  }
  return x;
};

const getTotalSelectedSurfacesNum = (surfaces) => {
  let count = countTrueAndFalseOnes(surfaces).trueOnes;
  return count;
};
